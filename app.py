...
for i in ax.patches:
    ax.text(i.get_width() + 0.1, i.get_y() + 0.5, formatar_brl(i.get_width()), ha='left', va='center')
...
# Adicionar valores formatados nos gráficos
for i in ax.patches:
    ax.text(i.get_x() + i.get_width() / 2, i.get_height() + 0.1, formatar_brl(i.get_height()), ha='center', va='bottom')
...
top_produtos = df.groupby('Produto')['Valor Total'].sum().sort_values(ascending=False).head(10)
fig, ax = plt.subplots(figsize=(10, 6))
